...
from axe3.schemas import Axe3DdeadInput, Axe3DdeadOutput
...
MODEL_PATH = Path(__file__).parent.parent / "models" / "best_model_ddead_artifact.pkl"

_bundle    = joblib.load(MODEL_PATH)
_pipeline  = _bundle["pipeline"]
_threshold = _bundle["threshold"]   # 0.20
...
# Identique à FDEAD — même features, même ordre
# ─────────────────────────────────────────────────────────────────────────────
...
def predict_ddead(data: Axe3DdeadInput) -> Axe3DdeadOutput:
    raw        = data.model_dump()
    X          = _build_feature_vector(raw)
    proba      = float(_model.predict_proba(X)[0][1])
    prediction = int(proba >= _threshold)

    return Axe3DdeadOutput(
